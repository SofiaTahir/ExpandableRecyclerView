...
public class ExpandableRecyclerAdapterHelper {
    ...
    public static List<Object> generateHeadingParentItemList(List<? extends HeadingListItem> headingItemList) {
        List<Object> headingWrapperList = new ArrayList<>();
        HeadingListItem headerListItem;
        ParentListItem parentListItem;
        HeaderWrapper headerWrapper;
        ParentWrapper parentWrapper;


        int headerListItemCount = headingItemList.size();
        for (int i = 0; i < headerListItemCount; i++) {
            headerListItem = headingItemList.get(i);
            headerWrapper = new HeaderWrapper(headerListItem);
            headingWrapperList.add(headerWrapper);

            List<? extends ParentListItem> parentItemList = (List<? extends ParentListItem>) headingItemList.get(i).getChildItemList();
            int parentListItemCount = parentItemList.size();
            for (int j = 0; j < parentListItemCount; j++) {
                parentListItem = parentItemList.get(j);
                parentWrapper = new ParentWrapper(parentListItem);
                headingWrapperList.add(parentWrapper);

                if (parentWrapper.isInitiallyExpanded()) {
                    parentWrapper.setExpanded(true);

                    int childListItemCount = parentWrapper.getChildItemList().size();
                    for (int k = 0; k < childListItemCount; k++) {
                        headingWrapperList.add(parentWrapper.getChildItemList().get(k));
                    }
                }
            }
        }

        return headingWrapperList;
    }
}
